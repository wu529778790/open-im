...
open-im is a **single-process** Node.js/TypeScript app with **no local infrastructure** (no DB, Docker, or Redis). It bridges **seven IM platforms** to **three AI CLIs** via a shared pipeline.
...
3. **`src/platform/handle-text-flow.ts`** — shared flow: access-control → chat mapping → command dispatch → AI enqueue.
...
- **clawbot/** — iLink API long-polling, image download+decrypt, message aggregation
...
`tool-adapter.interface.ts` is the common interface. `claude-sdk-adapter.ts` uses Claude Agent SDK **in-process** (default). `codex-adapter.ts` and `codebuddy-adapter.ts` spawn CLIs.

Key SDK options in `claude-sdk-adapter.ts`:
- `tools: { type: 'preset', preset: 'claude_code' }` — full tool set
- `skills: 'all'` — all skills enabled
- `INTERACTIVE_CONTEXT` injected into prompts — makes Claude behave interactively

### Commands & sessions

- **`src/commands/handler.ts`** — slash commands: `/help`, `/new`, `/sessions`, `/resume`, `/history`, `/delete`, `/rename`, `/fork`, `/models`, `/context`, `/plugins`, `/status`, `/cd`, `/pwd`, `/mode`
- **`src/session/session-manager.ts`** — per-user session state, persisted to `~/.open-im/data/sessions.json`

### Web dashboard (`web/`)

Single-page React app (Vite + TypeScript). Key components:
- `Dashboard.tsx` — main orchestrator (stats → platforms → config files)
- `SetupWizard.tsx` — first-run setup (Claude API → platforms → credentials → save+start)
- `PlatformCard.tsx` — collapsible platform config with test button and doc links
